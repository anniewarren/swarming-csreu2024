...
function pde_dns_implicit_amw(mu,nfouriermodes)
    ...
    %%%%%%%%%%%% system parameters %%%%%%%%%%%%%%%%%%
    L = pi; % domain size
    xgrid = linspace(-L, L, n)'; %column vector
    ygrid = linspace(-L,L,n); %row vector
    ...
    while t < t_max
        ...
        if mod(t,plotintervals)<dt % only plot at intervals of t_plot
            ...
            axis([xgrid(1) xgrid(end) ygrid(1) ygrid(end) 0 2]);
            clim([0,2]);
            colorbar;
...
function f = D(un1vec,un,n,dx,dy) 
    epsilon=1e-5; % artificial viscosity; small eps requires small dt
