...
function pde_dns_2d_implicit(mu,nfouriermodes)
    close all;
    ...
    iterations = 500;
    plotintervals = 0.05;
    epsilon=1e-1; % artificial viscosity; small eps requires small dt
    ...
    neglaplace = k.^2 + l.^2; % negative laplacian
    dx = 1i*k;
    dy = 1i*l;
    % define fourier coefficients of potential
    %vphys = cos(X) + cos(Y);
    %v = fft2(vphys)*4*pi*pi/(n*n); % normalize fourier transform by domain
    %area
    v=zeros(n,n); 
    v(2,2)=(n*n)/4;
    v(2,n)=(n*n)/4;
    ...
    u = u0*ones(n,n)+0.85*sin(X).*sin(Y)+0.6*cos(X).*cos(Y); %sine perturbation
    % 0.85*sin(X).*sin(Y)+0.6*cos(X).*cos(Y) for asymmetry
    ...
    while t < t_max
        ...
        if mod(t,plotintervals)<dt % only plot at intervals of t_plot
            ...
            drawnow;
            saveas(gcf,['sqbubbtofissmovie\n' num2str(n) '_mu' sprintf('%.4f', mu) '_iter' sprintf('%04d',iter) '.jpg']);
            iter = iter + 1;
        end
        ...
        F = fft2(-1*mu*ifft2(dx.*xcom + dy.*ycom,'symmetric')); 
        Fvec = reshape(F,n*n,1);
    
        D0 =@(w) w - dt.*D(w,uf,n,dx,dy,epsilon); % takes in a vector w, returns a vector
...
function f = D(un1vec,un,n,dx,dy,epsilon) 
